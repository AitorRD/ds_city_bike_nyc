...
```
### Viajes por año:
Vamos a visualizar si entre años existe algun patrón 
```{r}
library(ggplot2)

# Convertir starttime a formato fecha
df$starttime <- ymd_hms(df$starttime, tz="UTC")
...
# Eliminar registros con año NA
df <- df %>% filter(!is.na(year) & year >= 2015 & year <= 2021)
...
# Contar viajes por año y mes
df_counts <- df %>% 
  group_by(year, month, season) %>% 
  summarise(total_rides = n(), .groups = 'drop')

# Grid de gráficos por año con más espacio entre ellos y eje Y fijo
ggplot(df_counts, aes(x = month, y = total_rides, fill = season)) +
  geom_bar(stat = "identity") +
  facet_wrap(~year, scales = "fixed",nrow = 4 ,ncol = 3) +  # Escala fija en Y
  labs(title = "Número de viajes por mes y año (2015-2021)",
       x = "Mes", y = "Número de viajes", fill = "Estación") +
  ylim(0, 50000) +  # Fijar eje Y de 0 a 50,000
  theme_minimal() +
  theme(axis.text.x = element_blank(),
        strip.text = element_text(size = 14, face = "bold"),  # Aumentar tamaño de títulos de facetas
        panel.spacing = unit(2, "lines"),  # Mayor espacio entre gráficos
        text = element_text(size = 14))  # Aumentar tamaño general del gráfico
