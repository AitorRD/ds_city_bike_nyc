...
Para resolver esta pregunta calcularé las rutas más populares o transitadas y los representaré en un dashboard de shiny con filtros de gender, usertype y age, además de un filtro por día de la semana.

#### Funciones previas:

Previo al dashboard calculare las rutas ya que son muchos datos de golpe.
...
server <- function(input, output) {
  ...
  output$mapa_rutas <- renderLeaflet({
    ...
    if (nrow(rutas_populares) == 0) {
      return(leaflet() %>% addTiles() %>% addMarkers(lat = 40, lng = -74, label = "No hay rutas disponibles"))
    }
