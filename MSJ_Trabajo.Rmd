...
### Manuel Sánchez Jiménez - Pregunta 2 - ¿Se pueden encontrar zonas de trabajo?

```{r}
df_preprocesado %>%
  filter(starttime >= "2015-09-01" & starttime <= "2019-12-31")
```

```{r}
Q1 <- quantile(df_preprocesado$tripduration, 0.25, na.rm = TRUE)
Q3 <- quantile(df_preprocesado$tripduration, 0.75, na.rm = TRUE)
IQR <- Q3 - Q1

limite_inferior <- Q1 - 1.5 * IQR
limite_superior <- Q3 + 1.5 * IQR

df_sin_outliers <- df_preprocesado %>%
  filter(tripduration >= limite_inferior & tripduration <= 75)
```
...
df_filtrado <- df_sin_outliers %>%
  filter((2019 - birth.year) >= 16 & (2019 - birth.year) <= 68 & usertype == "Subscriber")

```

```{r}
df_morning <- df_filtrado %>%
  filter(hour(stoptime) >= 7 & hour(stoptime) <= 9) %>%  # Hora en el rango
  filter(!(hour(stoptime) == 9 & minute(stoptime) > 30))  # Excluir después de 09:30

df_evening <- df_filtrado %>%
  filter(hour(starttime) >= 17 & hour(starttime) <= 19) %>%  # Hora en el rango
  filter(!(hour(starttime) == 19 & minute(starttime) > 30))  # Excluir después de 19:30
...
head(df_morning %>% select(end.station.latitude, end.station.longitude, address))

map_morning <- leaflet(df_morning) %>%
  addProviderTiles(providers$CartoDB.Positron) %>%
  setView(lng = mean(df_morning$end.station.longitude),  # Centra en la media de las longitudes
          lat = mean(df_morning$end.station.latitude),   # Centra en la media de las latitudes
          zoom = 13) %>%  # Ajusta el nivel de zoom (más alto = más zoom)
...
head(df_evening %>% select(start.station.latitude, start.station.longitude, address))

map_evening <- leaflet(df_evening) %>%
  addProviderTiles(providers$CartoDB.Positron) %>%
  setView(lng = mean(df_evening$start.station.longitude),  # Centra en la media de las longitudes
          lat = mean(df_evening$start.station.latitude),   # Centra en la media de las latitudes
          zoom = 13) %>%  # Ajusta el nivel de zoom (más alto = más zoom)
