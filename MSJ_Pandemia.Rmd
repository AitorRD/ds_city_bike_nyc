...
```{r}
df_sin_outliers$year <- format(df_sin_outliers$starttime, "%Y")
df_avg_year <- df_sin_outliers %>%
  group_by(year) %>%
  summarise(media_viajes = mean(n()))
df_avg_year$year <- as.numeric(df_avg_year$year)
```
...
```{r}
df_sin_outliers$periodo <- case_when(
  df_sin_outliers$starttime < "2020-03-01" ~ "Pre-pandemia",
  df_sin_outliers$starttime >= "2020-03-12" ~ "Pandemia"
)

df_sin_outliers <- df_sin_outliers %>% filter(!is.na(periodo))

p1_graph <- ggplot(df_sin_outliers, aes(x = periodo, y = tripduration, fill = periodo)) +
  geom_violin(alpha = 0.7, trim = FALSE)
  theme_minimal()

p2_graph <- ggplot(df_sin_outliers, aes(x = periodo, y = tripduration, fill = periodo)) +
  geom_boxplot() +
...
grafico1 <- ggplot(df_compare, aes(x = ds)) +
  geom_line(aes(y = y, color = "Real"), size = 1) + 
  geom_line(aes(y = yhat, color = "Predicción sin pandemia"), size = 1, linetype = "dashed") +
  labs(title = "Comparación entre Predicción y Datos Reales",
       x = "Fecha", y = "Número de viajes") +
...
grafico2 <- ggplot(df_compare, aes(x = ds, y = diferencia)) +
  geom_col(fill = "red") +
  labs(title = "Impacto de la Pandemia en el Uso de Bicicletas",
